...
import { useAddMarket } from "@/lib/hooks/use-markets";
...
export default function AddMarketForm() {
  ...
  const addMarketMutation = useAddMarket();
  ...
  const onSubmit = async (data: MarketFormValues) => {
    // Show loading toast
    const loadingToast = toast.loading("Adding new market...");

    addMarketMutation.mutate(data, {
      onSuccess: () => {
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
          ...
          <CardFooter className="flex justify-between mt-5">
            ...
            <Button type="submit" disabled={addMarketMutation.isPending}>
              {addMarketMutation.isPending && (
