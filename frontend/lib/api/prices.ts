...
export interface MarketAverage {
  _id: string | { name: string; _id: string };
...
export const getPriceSummary = async (productId: string) => {
  try {
    const { data } = await axios.get(
      `${API_URL}/prices/price-summary/${productId}`
    );
...
export const bulkImportPrices = async (prices: Price[]) => {
  try {
    const { data } = await axios.post(`${API_URL}/prices/bulk-import`, {
      prices,
    });
    return data;
  } catch (error) {
...
export const getPriceVolatility = async (
  product: string,
  market: string,
  days = 30
) => {
...
export const analyzeSeasonalPrices = async (
  product: string,
  market: string,
  days = 365
) => {
...
export const analyzeCorrelations = async (
  ...
  try {
    ...
    const { data } = await axios.get(`${API_URL}/prices/correlations`, {
      params,
    });
    return data;
  } catch (error) {
