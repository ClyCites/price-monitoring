...
export const priceKeys = {
  ...
  trend: (product: string, market: string, days: number) =>
    [...priceKeys.trends(), product, market, days] as const,
  ...
  compareMarkets: (product: string) =>
    [...priceKeys.comparison(), product] as const,
  topMarkets: () => [...priceKeys.all, "topMarkets"] as const,
  topMarketsForProduct: (product: string) =>
    [...priceKeys.topMarkets(), product] as const,
  averages: () => [...priceKeys.all, "averages"] as const,
  averageByProduct: (product: string) =>
    [...priceKeys.averages(), product] as const,
  predictions: () => [...priceKeys.all, "predictions"] as const,
  prediction: (product: string, market: string) =>
    [...priceKeys.predictions(), product, market] as const,
};
...
export function usePriceSummary(productId: string) {
  return useQuery({
    queryKey: [...priceKeys.all, "summary", productId],
...
export function useUpdatePrice() {
  ...
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<Price> }) =>
      updatePrice(id, data),
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: priceKeys.detail(variables.id),
      });
      queryClient.invalidateQueries({ queryKey: priceKeys.lists() });
    },
    onError: (error: any) => {
